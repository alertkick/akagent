...
func TestExtractJSONStringField(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			data:     `{"StreamConfig":{},"State":{"Running":true},"ID":"4059c0fd00c757416e8448633426b154be59261794a9031e0766f6527a0b9053","Created":"2025-01-15T10:00:00Z","Path":"/entrypoint.sh","Name":"/alertpriority-web","Driver":"overlay2"}`,
			key:      "Name",
			expected: "/alertpriority-web",
		},
	}
...
func TestLookupDockerContainerName_TrimSlash(t *testing.T) {
	...
	name := "/alertpriority-web"
	trimmed := name[1:] // simulate strings.TrimPrefix(name, "/")
	if trimmed != "alertpriority-web" {
		t.Errorf("expected 'alertpriority-web', got %q", trimmed)
	}
}
